...
public class ConfirmServiceTest {
    ...
    @Test
    @DisplayName("쿠폰을 한 개 발급해서 DB에 하나의 내역이 저장되는지 확인한다. ")
    public void 쿠폰발급하나만() {
    ...
    public void 쿠폰1000개생성요청후100개만생성하기() throws InterruptedException{

        int threadCount = 1000; // 1000개 요청 동시에 넣기
        ...
        for (int i = 0; i < 1000; i++) {
            long userId =i ;
            executorService.submit(() -> {  // 람다 형식으로 각 쓰레드가 실행할 내역 작성
                try {
